...
export default function CountProducts({ product }: InitialPropsForCount) {
  // @ts-ignore
  const { cartItems, setCartItems }: IProduct[] = useContext(Context);
  const count = useMemo(
    () => cartItems?.find((item: any) => item.id === product.id)?.count,
    [cartItems]
  );
    ...
    setCartItems((cartItems: IProduct[]) => {
      return cartItems.map((storeItem: any) => {
        return {
          ...
          totalPrice:
            (storeItem.count - 1 > 1 ? Number(storeItem.count - 1) : 1) * Number(storeItem.price),
        };
      });
    });
  };
